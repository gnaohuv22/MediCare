...
public class DoctorDAO extends DBContext {
    ...
    public ArrayList<Doctor> getAllDoctors() {
        ...
        String sql = "                 SELECT d.*, b.[name] AS branchName, a.[name] AS ARName,  \n"
                ...
                + "				 Group by d.ARId, d.branchId , d.CVId , d.displayName , d.email , d.id , d.password ,d.phone , d.profilePicture ,d.salary , d.status , d.workplace, b.name,   a.name,  \n"
                + "                  Certificates,   Department.id, Department.name , CV.education, CV.introduce, CV.workHistory, CV.startYear ";
        try {
    ...
    public Doctor getDoctorById(String Id) {
        String SQL = "select id, email, displayName, branchId, phone, ARId, CVId, salary, workplace, profilePicture, [status], [password] from Doctor where id = ?";
        try ( PreparedStatement ps = connection.prepareStatement(SQL)) {
            ps.setString(1, Id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
...
//        System.out.println("concat: " + concatenateNames(""));
    ...
    public void updateDoctor(Doctor d) {
        try {
            ...
            statement.setString(11, d.getId());
            statement.executeUpdate();
        } catch (SQLException ex) {
        }
    }
