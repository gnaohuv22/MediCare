...
public class EmployeeDAO extends DBContext {
    ...
    public ArrayList<Employee> getAllEmployee() {
        ...
        try (PreparedStatement pstm = connection.prepareStatement(SQL)) {
            ...
            while (rs.next()) {
                ...
                Employee employee = new Employee(id, email, password, branch, name, birthDate, gender, address, workplace, province, phone, ethnic, employeeRole, createAt);
                list.add(employee);
            }
            return list;
        } catch (Exception e) {
    ...
    public ArrayList<Employee> getListEmployee(int offset, int fetch) {
        ArrayList<Employee> list = new ArrayList<>();
        String SQL = "SELECT Employee.id[eId], email, password, branchId, Employee.name[eName], birthDate, gender, address, workplace, provinceId, phone, ethnic, roleId, createAt," +
"       Branch.name[bName], Province.name[pName], EmployeeRole.role[erRole]" +
"       FROM Employee " +
...
"       join Province on Employee.provinceId=Province.id" +
"       join EmployeeRole on Employee.roleId=EmployeeRole.id" +
"	GROUP BY Employee.id, email, password, branchId, Employee.name, birthDate, gender, address, workplace, provinceId, phone, ethnic, roleId, createAt," +
...
//        " WHERE email = ?";
    ...
    public ArrayList<Employee> searchEmployee(String keyId, String keyName, String keyRoleId, int offset, int fetch) {
        ArrayList<Employee> list = new ArrayList<>();
        String SQL = "SELECT Employee.id[eId], email, password, branchId, Employee.name[eName], birthDate, gender, address, workplace, provinceId, phone, ethnic, roleId, createAt," +
        "Branch.name[bName], Province.name[pName], EmployeeRole.role[erRole] " +
        "FROM Employee " +
        ...
        "join Province on Employee.provinceId=Province.id "+
        "join EmployeeRole on Employee.roleId=EmployeeRole.id "+
        "WHERE Employee.id like ? AND Employee.name like ? AND EmployeeRole.id like ? " +
        "GROUP BY Employee.id, email, password, branchId, Employee.name, birthDate, gender, address, workplace, provinceId, phone, ethnic, roleId, createAt," +
"       Branch.name, Province.name, EmployeeRole.role " +
"	HAVING Employee.id IS NOT NULL" +
"       ORDER BY COUNT(Employee.id) DESC" +
